...
$\sum_{\lambda \in \sigma(F)} \mathbb{P}_{\psi}(\lambda) = \sum_{\lambda} \langle \psi | P_{\lambda} | \psi \rangle = \langle \psi | I | \psi \rangle = 1.$
...
$\langle F \rangle_{\psi} = \left\langle \psi \left| \sum_{\lambda} \lambda P_{\lambda} \right| \psi \right\rangle = \sum_{\lambda} \lambda \langle \psi | P_{\lambda} | \psi \rangle = \sum_{\lambda} \lambda \, \mathbb{P}_{\psi}(\lambda).$
...
$\Delta_{\psi}(F)^{2} := \left\langle \psi \left| \left(F - \langle F \rangle_{\psi} I\right)^{2} \right| \psi \right\rangle.$
...
$\begin{aligned} \Delta_{\psi}(F)^{2} &= \langle \psi | (F - \langle F \rangle_{\psi} I)^{2} | \psi \rangle \\ &= \langle \psi | F^{2} | \psi \rangle - 2 \langle F \rangle_{\psi} \langle \psi | F | \psi \rangle + \langle F \rangle_{\psi}^{2} \langle \psi | \psi \rangle \\ &= \langle F^{2} \rangle_{\psi} - \langle F \rangle_{\psi}^{2}. \end{aligned}$
...
$\Delta_{\psi}(F)^{2} = \|(F - \langle F \rangle_{\psi} I)|\psi\rangle\|^{2}.$
...
$\widehat{F} := F - \langle F \rangle_{\psi} I, \qquad \widehat{G} := G - \langle G \rangle_{\psi} I.$

Then

$\Delta_{\psi}(F)^{2}\Delta_{\psi}(G)^{2} \geq \frac{1}{4}\langle \{\widehat{F},\widehat{G}\} \rangle_{\psi}^{2} + \frac{1}{4}\left| \langle [F,G] \rangle_{\psi} \right|^{2},$

where $\{A,B\} := AB + BA$.

**Proof.** Set

$|f\rangle := \widehat{F}|\psi\rangle, \qquad |g\rangle := \widehat{G}|\psi\rangle.$

By Cauchy-Schwarz,

$\Delta_{\psi}(F)^{2}\Delta_{\psi}(G)^{2} = \|f\|^{2}\|g\|^{2} \geq |\langle f | g \rangle|^{2} = |\langle \psi | \widehat{F}\widehat{G} | \psi \rangle|^{2}.$

Now decompose

$\widehat{F}\widehat{G} = \frac{1}{2}\{\widehat{F},\widehat{G}\} + \frac{1}{2}[\widehat{F},\widehat{G}].$

The expectation of the anticommutator is real, while the expectation of the commutator is purely imaginary. Therefore,

$|\langle \widehat{F}\widehat{G} \rangle_{\psi}|^{2} = \frac{1}{4}\langle \{\widehat{F},\widehat{G}\} \rangle_{\psi}^{2} + \frac{1}{4}\left| \langle [\widehat{F},\widehat{G}] \rangle_{\psi} \right|^{2}.$
...
$i \frac{d}{dt}U(t,t_{0}) = H(t)U(t,t_{0}), \qquad U(t_{0},t_{0}) = I,$
...
$\begin{aligned} \frac{d}{dt}\left(U^{\dagger}U\right) &= \left(\frac{d}{dt}U^{\dagger}\right)U + U^{\dagger}\left(\frac{d}{dt}U\right) \\ &= iU^{\dagger}HU - iU^{\dagger}HU \\ &= 0. \end{aligned}$
...
$U(t) = e^{-itH}, \qquad |\psi(t)\rangle = e^{-itH}|\psi(0)\rangle.$
...
$\frac{d}{dt}U(t) = \lim_{s\to 0}\frac{U(t+s)-U(t)}{s} = U(t)\lim_{s\to 0}\frac{U(s)-I}{s} = U(t)A.$
...
$\mathbb{P}_{\rho}(\lambda) = \sum_{k} p_{k}\mathbb{P}_{\psi_{k}}(\lambda) = \sum_{k} p_{k}\langle \psi_{k}|P_{\lambda}|\psi_{k}\rangle.$

Using $\operatorname{tr}(|u\rangle\langle v|A)=\langle v|A|u\rangle$,

$\sum_{k} p_{k}\langle \psi_{k}|P_{\lambda}|\psi_{k}\rangle = \sum_{k} p_{k}\operatorname{tr}(|\psi_{k}\rangle\langle\psi_{k}|P_{\lambda}) = \operatorname{tr}\left(\sum_{k} p_{k} |\psi_{k}\rangle\langle\psi_{k}| P_{\lambda}\right) = \operatorname{tr}(\rho P_{\lambda}).$
...
$\langle F \rangle_{\rho} = \sum_{\lambda} \lambda \, \mathbb{P}_{\rho}(\lambda) = \sum_{\lambda} \lambda \, \operatorname{tr}(\rho P_{\lambda}) = \operatorname{tr}\left(\rho \sum_{\lambda}\lambda P_{\lambda}\right) = \operatorname{tr}(\rho F).$
...
$\rho_{\lambda} = \frac{P_{\lambda}\rho P_{\lambda}}{\operatorname{tr}(\rho P_{\lambda})}.$
...
$\mathcal{D}(\mathcal{H}) = \{\rho \in \mathcal{L}(\mathcal{H}) : \rho \geq 0,\ \operatorname{tr}(\rho)=1\}.$
...
$\operatorname{tr}(\rho) = \sum_{k} p_{k}\operatorname{tr}(|\psi_{k}\rangle\langle\psi_{k}|) = \sum_{k} p_{k} = 1.$
...
$\operatorname{tr}(\rho_{1}F)=\operatorname{tr}(\rho_{2}F) \quad \forall F=F^{\dagger}.$
...
$\rho \text{ is pure } \iff \operatorname{tr}(\rho^{2})=\operatorname{tr}(\rho).$
...
$0 = \sum_{k}\lambda_{k} - \sum_{k}\lambda_{k}^{2} = \sum_{k}\lambda_{k}(1-\lambda_{k}).$
...
$\sqrt{p_{i}}|\psi_{i}\rangle = \sum_{j=1}^{n} U_{ij}\sqrt{\lambda_{j}}|e_{j}\rangle \qquad \text{for all } i.$
...
$\rho(t) = \sum_{k} p_{k} |\psi_{k}(t)\rangle\langle\psi_{k}(t)| = \sum_{k} p_{k} U|\psi_{k}(t_{0})\rangle\langle\psi_{k}(t_{0})|U^{\dagger} = U\rho(t_{0})U^{\dagger}.$
...
$\mathcal{H}_{\mathrm{fermionic}} = \bigwedge^{N}\mathbb{C}^{d}, \qquad \mathcal{H}_{\mathrm{bosonic}} = \bigvee^{N}\mathbb{C}^{d}.$
...
$\mathcal{H}_{1}=\operatorname{span}\{|e_{1}\rangle,\dots,|e_{d_{1}}\rangle\}, \qquad \mathcal{H}_{2}=\operatorname{span}\{|f_{1}\rangle,\dots,|f_{d_{2}}\rangle\},$

and let

$|\Psi\rangle = \sum_{i=1}^{d_{1}}\sum_{j=1}^{d_{2}} C_{ij}|e_{i}\rangle\otimes|f_{j}\rangle.$

**Definition (Reduced state).** The state of subsystem $1$ is the unique density matrix $\rho^{(1)}$ such that

$\operatorname{tr}(\rho^{(1)}F_{1}) = \langle \Psi | (F_{1}\otimes I)|\Psi\rangle \qquad \forall F_{1}\in \operatorname{Herm}(\mathcal{H}_{1}).$

Similarly, $\rho^{(2)}$ is defined by

$\operatorname{tr}(\rho^{(2)}F_{2}) = \langle \Psi | (I\otimes F_{2})|\Psi\rangle \qquad \forall F_{2}\in \operatorname{Herm}(\mathcal{H}_{2}).$
...
$\rho^{(1)} = CC^{\dagger}, \qquad \rho^{(2)} = C^{T}\overline{C}.$

**Proof.** For subsystem $1$,

$\begin{aligned} \langle \Psi | (F_{1}\otimes I)|\Psi\rangle &= \sum_{i,k=1}^{d_{1}}\sum_{j=1}^{d_{2}} \overline{C}_{kj}C_{ij}\langle e_{k}|F_{1}|e_{i}\rangle \\ &= \sum_{i,k=1}^{d_{1}} \left(\sum_{j=1}^{d_{2}} C_{ij}\overline{C}_{kj}\right) \langle e_{k}|F_{1}|e_{i}\rangle. \end{aligned}$

Hence the matrix elements are

$\langle e_{i}|\rho^{(1)}|e_{k}\rangle = \sum_{j=1}^{d_{2}} C_{ij}\overline{C}_{kj} = (CC^{\dagger})_{ik}.$

The second formula is obtained similarly:

$\langle f_{j}|\rho^{(2)}|f_{l}\rangle = \sum_{i=1}^{d_{1}} C_{ij}\overline{C}_{il} = (C^{T}\overline{C})_{jl}.$
...
$\rho^{(1)} = |\psi_{1}\rangle\langle\psi_{1}|, \qquad \rho^{(2)} = |\psi_{2}\rangle\langle\psi_{2}|.$
...
$\langle i_{k}|\rho^{(k)}|j_{k}\rangle = \sum_{i_{1},\dots,\widehat{i_{k}},\dots,i_{N}} \langle i_{1}\cdots i_{k}\cdots i_{N}|\rho|i_{1}\cdots j_{k}\cdots i_{N}\rangle,$
...
$\operatorname{tr}(\rho^{(k)}F_{k}) = \operatorname{tr}(\rho \widetilde{F}_{k}),$

where

$\widetilde{F}_{k} = I\otimes\cdots\otimes I \otimes F_{k}\otimes I\otimes\cdots\otimes I.$
...
$\widetilde{\rho}_{1}=U\rho_{1}U^{\dagger}, \qquad \widetilde{\rho}_{2}=V\rho_{2}V^{\dagger}.$

**Proof.** For every observable $F_{1}$ on subsystem $1$,

$\begin{aligned} \operatorname{tr}(\widetilde{\rho}_{1}F_{1}) &= \langle \widetilde{\Psi}|F_{1}\otimes I|\widetilde{\Psi}\rangle \\ &= \langle \Psi|U^{\dagger}F_{1}U \otimes I|\Psi\rangle \\ &= \operatorname{tr}(\rho_{1}U^{\dagger}F_{1}U) = \operatorname{tr}(U\rho_{1}U^{\dagger}F_{1}). \end{aligned}$
...
$\rho_{1} = \sum_{i=1}^{r} p_{i}|u_{i}\rangle\langle u_{i}|, \qquad \rho_{2} = \sum_{i=1}^{r} p_{i}|v_{i}\rangle\langle v_{i}|.$
...
$|\Psi_{\max}\rangle = \frac{1}{\sqrt{d}}\sum_{i=0}^{d-1}|i\rangle\otimes|i\rangle.$
...
$\rho_{\mathrm{after}} = \sum_{\lambda} P_{\lambda}|\psi\rangle\langle\psi|P_{\lambda}.$
...
$\begin{aligned} \operatorname{tr}_{1}(A\otimes B) &= \sum_{i} (\langle i|\otimes I)(A\otimes B)(|i\rangle\otimes I) \\ &= \sum_{i}\langle i|A|i\rangle\,B \\ &= \operatorname{tr}(A)\,B. \end{aligned}$

$\square$

**Lemma (Cyclicity in the traced subsystem).** If $X$ acts only on subsystem $1$, then

$\operatorname{tr}_{1}[(X\otimes I)Y] = \operatorname{tr}_{1}[Y(X\otimes I)].$
...
$\begin{aligned} \rho'_{B} &= \operatorname{tr}_{A}(\rho') \\ &= \sum_{\lambda}\operatorname{tr}_{A}\left((P_{\lambda}\otimes I)\rho(P_{\lambda}\otimes I)\right) \\ &= \sum_{\lambda}\operatorname{tr}_{A}\left((P_{\lambda}^{2}\otimes I)\rho\right) \\ &= \operatorname{tr}_{A}\left(\left(\sum_{\lambda}P_{\lambda}\otimes I\right)\rho\right) \\ &= \operatorname{tr}_{A}(\rho) = \rho_{B}. \end{aligned}$
...
$\Phi:\mathbb{R}\to SO(2), \qquad \Phi(\theta)= \begin{pmatrix} \cos\theta & -\sin\theta \\ \sin\theta & \cos\theta \end{pmatrix},$
...
$\operatorname{Lie}(G) := \{X\in M_{n}(\mathbb{C}) : e^{tX}\in G \text{ for all } t\in\mathbb{R}\}.$
...
$\operatorname{Lie}(U(n))=\mathfrak{u}(n) = \{X\in M_{n}(\mathbb{C}) : X^{\dagger}=-X\},$

$\operatorname{Lie}(SU(n))=\mathfrak{su}(n) = \{X\in M_{n}(\mathbb{C}) : X^{\dagger}=-X,\ \operatorname{tr}(X)=0\},$

$\operatorname{Lie}(O(n))=\mathfrak{o}(n) = \{X\in M_{n}(\mathbb{R}) : X^{T}=-X\},$

$\operatorname{Lie}(SO(n))=\mathfrak{so}(n) = \{X\in M_{n}(\mathbb{R}) : X^{T}=-X\}.$
...
$g(t)=e^{tX} \qquad \forall t\in\mathbb{R}.$

**Theorem (Differential of a Lie group homomorphism).** Let $\Phi:G\to H$ be a Lie group homomorphism, and let

$\mathfrak{g}=\operatorname{Lie}(G), \qquad \mathfrak{h}=\operatorname{Lie}(H).$
...
$\operatorname{Ad}_{g}:\mathfrak{g}\to\mathfrak{g}, \qquad \operatorname{Ad}_{g}(X)=gXg^{-1}.$
...
$\operatorname{Ad}:G\to GL(\mathfrak{g}), \qquad g\mapsto \operatorname{Ad}_{g},$
...
$\operatorname{ad}_{X}(Y) := \left.\frac{d}{dt}\right|_{t=0}\operatorname{Ad}_{e^{tX}}(Y).$
...
$\operatorname{ad}_{X}(Y) = \left.\frac{d}{dt}\right|_{t=0} e^{tX}Ye^{-tX} = XY-YX.$
...
$e^{X}Ye^{-X} = Y+[X,Y]+\frac{1}{2!}[X,[X,Y]]+\cdots.$
...
$(X|Y)_{HS} := d\,\operatorname{tr}(X^{\dagger}Y) = -d\,\operatorname{tr}(XY).$
...
$\begin{aligned} (\operatorname{Ad}_{U}(X)|\operatorname{Ad}_{U}(Y))_{HS} &= -d\,\operatorname{tr}(UXU^{-1}UYU^{-1}) \\ &= -d\,\operatorname{tr}(UXYU^{-1}) \\ &= -d\,\operatorname{tr}(XY) \\ &= (X|Y)_{HS}. \end{aligned}$
...
$[\operatorname{Ad}_{U}]_{ij} = (\gamma_{i}|\operatorname{Ad}_{U}(\gamma_{j}))_{HS} = -d\,\operatorname{tr}(\gamma_{i}U\gamma_{j}U^{-1}).$
...
$\sigma_{x}= \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}, \qquad \sigma_{y}= \begin{pmatrix} 0 & -i \\ i & 0 \end{pmatrix}, \qquad \sigma_{z}= \begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}, \qquad I= \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}.$
...
$\{\sigma_{j},\sigma_{k}\} = 2\delta_{jk}I.$

**Remark.** Each Pauli matrix is both Hermitian and unitary:

$\sigma_{j}^{\dagger}=\sigma_{j}, \qquad \sigma_{j}^{2}=I.$

### 9.2 The Lie algebra $\mathfrak{su}(2)$

**Definition.** Set

$\widetilde{\sigma}_{k}:=-\frac{i}{2}\sigma_{k}, \qquad k=1,2,3.$

Then $\widetilde{\sigma}_{k}\in \mathfrak{su}(2)$.

**Lemma.**

$[\widetilde{\sigma}_{j},\widetilde{\sigma}_{k}] = \sum_{l=1}^{3}\epsilon_{jkl}\widetilde{\sigma}_{l}.$

**Corollary.**

$\mathfrak{su}(2) = \operatorname{span}_{\mathbb{R}}\{\widetilde{\sigma}_{1},\widetilde{\sigma}_{2},\widetilde{\sigma}_{3}\}.$

### 9.3 Isomorphism with $\mathbb{R}^{3}$

**Definition.** For $\vec{a}=(a_{1},a_{2},a_{3})\in\mathbb{R}^{3}$, write

$\vec{a}\cdot\widetilde{\vec{\sigma}} := a_{1}\widetilde{\sigma}_{1}+a_{2}\widetilde{\sigma}_{2}+a_{3}\widetilde{\sigma}_{3}.$

**Theorem.** The map

$\mathbb{R}^{3}\to \mathfrak{su}(2), \qquad \vec{a}\mapsto \vec{a}\cdot\widetilde{\vec{\sigma}},$

is a Lie algebra isomorphism. More precisely,

1.

$(\vec{a}\cdot\widetilde{\vec{\sigma}} \mid \vec{b}\cdot\widetilde{\vec{\sigma}})_{HS} = \vec{a}\cdot\vec{b},$

2.

$[\vec{a}\cdot\widetilde{\vec{\sigma}},\vec{b}\cdot\widetilde{\vec{\sigma}}] = (\vec{a}\times\vec{b})\cdot\widetilde{\vec{\sigma}}.$
...
$e^{-i\frac{\alpha}{2}\hat{n}\cdot\vec{\sigma}} = \cos\left(\frac{\alpha}{2}\right)I - i\sin\left(\frac{\alpha}{2}\right)\hat{n}\cdot\vec{\sigma}.$

**Theorem.** Every element of $SU(2)$ can be written as

$U=e^{\alpha \hat{n}\cdot\widetilde{\vec{\sigma}}} = e^{-i\frac{\alpha}{2}\hat{n}\cdot\vec{\sigma}}$
...
$\sigma_{z}|0\rangle=|0\rangle, \qquad \sigma_{z}|1\rangle=-|1\rangle.$
...
$\rho_{\vec{x}} = \frac{1}{2}\left(I+\vec{x}\cdot\vec{\sigma}\right), \qquad \vec{x}\in\mathbb{R}^{3},$
...
$\rho_{\vec{x}}^{2} = \frac{1}{4}(I+2\vec{x}\cdot\vec{\sigma}+|\vec{x}|^{2}I).$
...
$\rho_{\vec{x}}=\frac{1}{2}(I+\vec{x}\cdot\vec{\sigma}), \qquad U\in SU(2).$

Then there exists a rotation $R_{U}\in SO(3)$ such that

$U\rho_{\vec{x}}U^{\dagger} = \frac{1}{2}\left(I+(R_{U}\vec{x})\cdot\vec{\sigma}\right).$

**Proof.** Conjugation preserves the identity and acts linearly on the span of the Pauli matrices:

$U\rho_{\vec{x}}U^{\dagger} = \frac{1}{2}I+\frac{1}{2}U(\vec{x}\cdot\vec{\sigma})U^{\dagger}.$
...
$U(\vec{x}\cdot\vec{\sigma})U^{\dagger} = (R_{U}\vec{x})\cdot\vec{\sigma}.$
...
$\rho = \frac{1}{d}\left(I+i\sum_{k=1}^{d^{2}-1}x_{k}\gamma_{k}\right) = \frac{1}{d}\left(I+i\,\vec{x}\cdot\vec{\gamma}\right),$
...
$\rho = \frac{1}{d}(I+i\,\vec{x}\cdot\vec{\gamma}), \qquad U\in SU(d).$

Then

$U\rho U^{\dagger} = \frac{1}{d}\left(I+i\,(\operatorname{Ad}_{U}\vec{x})\cdot\vec{\gamma}\right),$
...
$U\rho U^{\dagger} = \frac{1}{d}I+\frac{i}{d}U(\vec{x}\cdot\vec{\gamma})U^{\dagger}.$
